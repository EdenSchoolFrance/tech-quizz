    ...
    <div class="w-full max-w-6xl flex flex-col justify-between gap-8">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center">
                <img class="w-[56px] h-[56px] me-4"
                     src="/build/assets/{{ $quizz->img_url }}"
                     alt="Quiz Image">
                <p class="font-[500] text-[#313E51] text-[28px] font-rubik">{{ $quizz->title }}</p>
            </div>
            <a href="/dashboard">
                <svg width="15"
                     height="16"
        ...
        <div class="flex flex-wrap justify-between">
            <div class="w-[45%] h-[430px] flex flex-col justify-between">
                <div class="textDiv">
                    <p class="text-[20px] text-gray-500 italic font-rubik">
                        Question {{ $question->order }} of 10
                    </p>
                    <p class="text-[36px] text-[#313E51] font-[500] font-rubik">
            ...
            <div class="w-[45%]">
                <form class="answerForm h-full mb-0 flex justify-between flex-col gap-8"
                    ...
                    <button type="submit"
                            id="submitBtn"
                            class="font-rubik text-white text-[22px] bg-purple text-center py-5 rounded-[24px] cursor-pointer"
                    >Submit answer
                    </button>
                    <p class="text-[#EE5454] text-center font-[500] font-rubik"
                       id="errorMessage"></p>
                </form>
            </div>
        </div>
    ...
    document.addEventListener("DOMContentLoaded", function (event) {
        ...
        answerDivs.forEach(div => {
            div.addEventListener('click', function () {
                ...
                const order = div.querySelector(".order");
                if (input.disabled) {
                    return;
                }
                orders2.forEach(o => o.classList.remove('bg-[#A729F5]', 'text-white'))
                answerDivs.forEach(d => d.classList.remove('active'));
                div.classList.add('active');
                order.classList.add('bg-[#A729F5]', 'text-white');
            });
        });
        ...
        // answerDivs.forEach(function (divs) {
        //     divs.addEventListener('mouseenter', function () {
        //         const order = divs.querySelector(".order");
        //         if(order.classList.contains('bg-[#A729F5] text-white')) {
        //             order.classList.add('bg-[#A729F5]', 'text-white')
        //         } else {
        //             order.classList.add('bg-[#F6E7FF]', 'text-[#A729F5]')
        //         }
        //     })
        //
        ...
        //             order.classList.add('bg-[#A729F5]', 'text-white')
        //         } else {
        //             order.classList.remove('bg-[#F6E7FF]', 'text-[#A729F5]')
        //         }
        //     })
        // })
